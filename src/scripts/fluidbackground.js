  (()=>{
    ...
    ['input','change'].forEach(evt=>{
      ...
      ui.res.addEventListener(evt, ()=>{ syncLabels();
    // --- Modal open/close helpers ---
    const openModal = () => {
      if (!ui.modal) return;
      ui.modal.classList.add('open');
      if (ui.modalBackdrop) ui.modalBackdrop.classList.add('open');
      ui.modal.setAttribute('aria-hidden', 'false');
      // Focus the dialog for accessibility
      const dlg = ui.modal.querySelector('.modal-dialog');
      if (dlg) dlg.focus();
    };
    const closeModal = () => {
      if (!ui.modal) return;
      ui.modal.classList.remove('open');
      if (ui.modalBackdrop) ui.modalBackdrop.classList.remove('open');
      ui.modal.setAttribute('aria-hidden', 'true');
      // Return focus to the toggle button
      if (ui.panelToggle) ui.panelToggle.focus();
    };

    // Toggle button opens the modal (formerly "Hide settings")
    if (ui.panelToggle) {
      ui.panelToggle.addEventListener('click', (e) => {
        const isOpen = ui.modal && ui.modal.classList.contains('open');
    ...
    if (ui.modalBackdrop) ui.modalBackdrop.addEventListener('click', closeModal);
    // ESC key closes
 ...
 resize(); });
    ...
    canvas.addEventListener('touchstart', e=>{ pointer.down = 1;
    // Also listen on the foreground app grid so hover there still drives the fluid effect
    const appGrid = document.querySelector('.app-grid-section');
    if (appGrid) {
      appGrid.addEventListener('mousemove', onMove);
      appGrid.addEventListener('mousedown', onDown);
      appGrid.addEventListener('mouseenter', ()=>{ pointer.over = 1; });
      appGrid.addEventListener('mouseleave', ()=>{ pointer.over = 0; onUp(); });
      appGrid.addEventListener('touchmove', onMove, {passive:true});
      appGrid.addEventListener('touchstart', e=>{ pointer.down = 1; pointer.over = 1; onMove(e); }, {passive:true});
    }
 pointer.over = 1; onMove(e); }, {passive:true});
